...
class SmartLogger:
    ...
    def __init__(self, 
        ...
        self.file_output = self._get_env_variable(
            str(file_output) if file_output is not None else None, "FILE_OUTPUT", "True"
        ) == "True"
        ...
        os.makedirs(os.path.dirname(self.blacklisted_log_path), exist_ok=True)
        os.makedirs(self.blacklisted_detail_log_dir, exist_ok=True)
    ...
    def log(self, level, message, category=None, params=None, max_inline_chars=100):
        ...
        if params:
            ...
            if len(params_str) <= max_inline_chars:
