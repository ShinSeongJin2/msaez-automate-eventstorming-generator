from eventstorming_generator.models import MergeCreatedBoundedContextGeneratorOutput
from eventstorming_generator.terminal.commons.generator import execute_merge_created_bounded_context_generator

class TestMergeCreatedBoundedContextGenerator:
    """MergeCreatedBoundedContextGenerator 클래스의 테스트"""
    def test_merge_created_bounded_context_generator(self):
        """MergeCreatedBoundedContextGenerator를 테스트"""
        generator_result:MergeCreatedBoundedContextGeneratorOutput = execute_merge_created_bounded_context_generator(is_save_to_temp=False)

        # 기본 검증
        assert generator_result is not None, "생성 결과가 None이면 안 됩니다"
        assert hasattr(generator_result, 'mergedBoundedContexts'), "mergedBoundedContexts 속성이 존재해야 합니다"
        assert isinstance(generator_result.mergedBoundedContexts, list), "mergedBoundedContexts는 리스트여야 합니다"
        
        # Bounded Context 개수 검증 (3-15개 사이여야 함)
        bounded_contexts_count = len(generator_result.mergedBoundedContexts)
        assert 3 <= bounded_contexts_count <= 15, \
            f"Bounded Context 개수는 3-15개 사이여야 합니다. 현재: {bounded_contexts_count}개"
        
        # 각 Bounded Context 검증
        for idx, bc in enumerate(generator_result.mergedBoundedContexts):
            # 필수 필드 존재 확인
