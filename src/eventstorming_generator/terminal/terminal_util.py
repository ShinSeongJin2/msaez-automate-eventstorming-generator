...
class TerminalUtil:
    ...
    def save_es_summarize_result_to_temp_file(es_value: Any, file_name: str) -> None:
        ...
        TerminalUtil.save_dict_to_temp_file(summarized_es_value, f"{file_name}_es_value_summarized")
    ...
    def check_error_logs_from_state(state: State, file_name: str) -> None:
        ...
        if len(error_logs) > 0:
            ...
            TerminalUtil.save_dict_to_temp_file(error_logs, f"{file_name}_error_logs")
        else:
    ...
    def run_generator(generator: XmlBaseGenerator, inputs: dict, model_type: str = "normal") -> dict:
        try:
            ...
            TerminalUtil.save_dict_to_temp_file(entire_prompt, f"run_input_{generator.__class__.__name__}")
            TerminalUtil.save_dict_to_temp_file(generator_output, f"run_output_{generator.__class__.__name__}")
