...
class EsActionsUtil:
    ...
    def apply_actions(es_value: EsValueModel | Dict[str, Any], actions: List[ActionModel], 
        ...
        for action in sorted_actions:
            EsActionsUtil._apply_action(action, user_id, project_id, es_dict, callbacks)
        
        for callback in callbacks["afterAllObjectAppliedCallBacks"]:
            callback(es_dict, user_id, project_id)
    ...
    @staticmethod
    def _apply_action(action: ActionModel, user_id: str, project_id: str, 
        ...
        if action.objectType == "BoundedContext":
            BoundedContextProcessor.get_action_applied_es_value(
                action, user_id, project_id, es_value, callbacks
            )
