...
The approximate structure is as follows.
{
    ...
    def get_summarized_es_value(es_value: Dict[str, Any], keys_to_exclude_filter: List[str] = None, es_alias_trans_manager: Optional[EsAliasTransManager] = None) -> Dict[str, Any]:
        ...
        summarized_bounded_contexts = ESValueSummarizeWithFilter._sort_dict_list(summarized_bounded_contexts)
    ...
    def get_summarized_bounded_context_value(es_value: Dict[str, Any], bounded_context: Dict[str, Any], 
        ...
        aggregates = ESValueSummarizeWithFilter._sort_dict_list(aggregates)
    ...
    def get_summarized_actor_value(es_value: Dict[str, Any], bounded_context: Dict[str, Any], 
        ...
        if not ESValueSummarizeWithFilter._check_key_filters(keys_to_exclude_filter, ["name", "actors.name"]):
            unique_actors = get_unique_actors(actors, "name")
            return ESValueSummarizeWithFilter._sort_dict_list(unique_actors, ["name"])
        
        return ESValueSummarizeWithFilter._sort_dict_list(actors, ["name"])
    ...
    def get_summarized_entity_value(aggregate: Dict[str, Any], keys_to_exclude_filter: List[str] = None, 
        ...
        return ESValueSummarizeWithFilter._sort_dict_list(summarized_entity_value)
    ...
    def get_summarized_enumeration_value(aggregate: Dict[str, Any], keys_to_exclude_filter: List[str] = None, 
        ...
        for element in aggregate["aggregateRoot"]["entities"]["elements"].values():
            if element and element.get("_type") == 'org.uengine.uml.model.enum':
                ...
                if element.get("items"):
                    raw_items = [item.get("value") for item in element["items"]]
                    sorted_items = ESValueSummarizeWithFilter._sort_simple_list(raw_items)
                    enum.update(get_conditional_value(
        ...
        return ESValueSummarizeWithFilter._sort_dict_list(summarized_enumeration_value)
    ...
    def get_summarized_value_object_value(aggregate: Dict[str, Any], keys_to_exclude_filter: List[str] = None, 
        ...
        return ESValueSummarizeWithFilter._sort_dict_list(summarized_value_object_value)
    ...
    def get_summarized_command_value(es_value: Dict[str, Any], bounded_context: Dict[str, Any], 
        ...
        def get_output_events(element):
            ...
            return ESValueSummarizeWithFilter._sort_dict_list(events)
        ...
        return ESValueSummarizeWithFilter._sort_dict_list(summarized_command_value)
    ...
    def get_summarized_policy_value(es_value: Dict[str, Any], bounded_context: Dict[str, Any], 
        ...
        for element in es_value.get("elements", {}).values():
            if (element and element.get("_type") == 'org.uengine.modeling.model.Policy' and
                ...
                input_events = ESValueSummarizeWithFilter._sort_dict_list(input_events)
                policy.update(get_conditional_value(
                ...
                output_events = ESValueSummarizeWithFilter._sort_dict_list(output_events)
                policy.update(get_conditional_value(
        ...
        return ESValueSummarizeWithFilter._sort_dict_list(summarized_policy_value)
    ...
    def get_summarized_event_value(es_value: Dict[str, Any], bounded_context: Dict[str, Any], 
        ...
        return ESValueSummarizeWithFilter._sort_dict_list(summarized_event_value)
    ...
    def get_summarized_read_model_value(es_value: Dict[str, Any], bounded_context: Dict[str, Any], 
        ...
        return ESValueSummarizeWithFilter._sort_dict_list(summarized_read_model_value)

    @staticmethod
    def _sort_dict_list(items: List[Dict[str, Any]], primary_keys: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        """
        주어진 딕셔너리 리스트를 지정된 키 순서에 따라 정렬합니다.

        Args:
            items: 정렬할 딕셔너리 리스트
            primary_keys: 정렬에 사용할 키 목록. 기본값은 ["name", "id"].
        ...
        primary_keys = primary_keys or ["name", "id"]

        def _create_sort_key(item: Dict[str, Any]) -> tuple:
            sort_values = []
            for key in primary_keys:
                value = item.get(key)
                if isinstance(value, str):
                    sort_values.append(value.lower())
                elif value is None:
                    sort_values.append("")
                else:
        ...
        return sorted(items, key=_create_sort_key)

    @staticmethod
    def _sort_simple_list(values: List[Any]) -> List[Any]:
        """
        문자열 또는 기타 단순 값 리스트를 일관된 방식으로 정렬합니다.
        ...
        return sorted(values, key=lambda val: ("" if val is None else str(val)).lower())
    ...
    def _get_summarized_field_descriptors(field_descriptors: List[Dict[str, Any]], on_after_create_property: Callable = None) -> List[Dict[str, Any]]:
        ...
        return ESValueSummarizeWithFilter._sort_dict_list(properties, ["name"])
