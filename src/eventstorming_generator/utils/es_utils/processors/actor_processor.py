...
class ActorProcessor:
    ...
    def get_action_applied_es_value(action: Dict[str, Any], user_id: str, project_id: str, 
        ...
        if action.type == "create":
            ActorProcessor._create_actor(action, user_id, project_id, es_value, callbacks)
    
    @staticmethod
    def _create_actor(action: Dict[str, Any], user_id: str, project_id: str, es_value: Dict[str, Any], 
        ...
        # Actor 객체 생성
        actor_object = ActorProcessor._get_actor_base(user_id, actor_name, bounded_context_id, 0, 0, actor_id)
        
        # 위치 설정
        if action.args.get("relatedCommandId"):
            def set_position(es_value: Dict[str, Any], user_id: str, project_id: str) -> None:
    ...
    def make_actor_to_element(es_value: Dict[str, Any], action: Dict[str, Any], 
                            element_object: Dict[str, Any], user_id: str) -> None:
        """Element와 연결된 Actor를 생성합니다"""          
        if not action.args.get("actor"):
            return
            
        actor_base = ActorProcessor._get_actor_base(
            user_id, action.args.get("actor"), action.ids.get("boundedContextId"), 0, 0
        )
    ...
    @staticmethod
    def _get_actor_base(user_id: str, actor_name: str, 
