...
class CommandProcessor:
    ...
    def get_action_applied_es_value(action: Dict[str, Any], user_id: str, project_id: str, 
        ...
        if action.type == "create":
            CommandProcessor._create_command(action, user_id, project_id, es_value, callbacks)
    
    @staticmethod
    def _create_command(action: Dict[str, Any], user_id: str, project_id: str, 
                        es_value: Dict[str, Any], callbacks: Dict[str, List]) -> None:
        ...
        command_object = CommandProcessor._get_command_base(
            user_id, command_name, command_alias, api_verb, [], is_rest_repository,
            bounded_context_id, aggregate_id, 0, 0, command_id, refs
        )
        
        # 출력 이벤트 설정
        if action.args.get("outputEventIds"):
            def set_output_events(es_value: Dict[str, Any], user_id: str, project_id: str) -> None:
        ...
        def make_ui_and_actor_to_command(es_value: Dict[str, Any], user_id: str, project_id: str) -> None:
            from .ui_processor import UIProcessor
            from .actor_processor import ActorProcessor
            
            ui_object = UIProcessor.make_ui_to_element(es_value, action, command_object, user_id)
    ...
    @staticmethod
    def _get_command_base(user_id: str, name: str, display_name: str, 
    ...
    def _make_command_to_event_relation(command_object: Dict[str, Any], 
        ...
        def create_relations(es_value: Dict[str, Any], user_id: str, project_id: str) -> None:
