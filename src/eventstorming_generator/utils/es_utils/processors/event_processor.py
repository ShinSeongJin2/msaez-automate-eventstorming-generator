...
class EventProcessor:
    ...
    def get_action_applied_es_value(action: Dict[str, Any], user_id: str, project_id: str, 
        ...
        if action.get("type") == "create":
            EventProcessor._create_event(action, user_id, project_id, es_value, callbacks)
        elif action.get("type") == "update":
            EventProcessor._update_event(action, user_id, project_id, es_value, callbacks)
    
    @staticmethod
    def _create_event(action: Dict[str, Any], user_id: str, project_id: str, es_value: Dict[str, Any], 
        ...
        event_object = EventProcessor._get_event_base(
            user_id, event_name, event_alias, bounded_context_id, aggregate_id, 0, 0, event_id, refs
        )
        ...
        # 필드 생성을 위한 콜백 등록
        def set_field_descriptors_callback(es_value_cb: Dict[str, Any], user_id: str, project_id: str) -> None:
    ...
    @staticmethod
    def _update_event(action: Dict[str, Any], user_id: str, project_id: str, 
                   es_value: Dict[str, Any], callbacks: Dict[str, List]) -> None:
        """기존 Event를 업데이트합니다"""
        event_id = action.get("ids", {}).get("eventId", "")
    ...
    @staticmethod
    def _get_event_base(user_id: str, name: str, display_name: str, 
