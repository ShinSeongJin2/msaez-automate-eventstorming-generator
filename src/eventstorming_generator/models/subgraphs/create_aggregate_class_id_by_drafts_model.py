...
class CreateAggregateClassIdByDraftsModel(BaseModelWithItem):
    ...
    worker_generations: Dict[str, ClassIdGenerationState] = Field(default_factory=dict)
    current_batch: List[ClassIdGenerationState] = Field(default_factory=list)
    parallel_worker_results: List[ClassIdGenerationState] = Field(default_factory=list)
